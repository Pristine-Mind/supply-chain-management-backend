...
from market.models import MarketplaceProduct, MarketplaceSale, PaymentStatus, SaleStatus
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--count", type=int, default=500, help="Number of marketplace sales to create (default: 500)")
        parser.add_argument(
            "--anonymous-ratio", type=float, default=0.3, help="Ratio of anonymous buyers (0.0-1.0, default: 0.3)"
        )
        parser.add_argument(
            "--status-distribution",
            type=str,
        ...
        parser.add_argument("--date-range-days", type=int, default=90, help="Create sales within last N days (default: 90)")
        parser.add_argument(
        ...
        parser.add_argument("--batch-size", type=int, default=100, help="Batch size for bulk creation (default: 100)")
    ...
    def handle(self, *args, **options):
        ...
        try:
            pending_pct, completed_pct, cancelled_pct, refunded_pct = map(int, options["status_distribution"].split(","))
        ...
        currency_choices = self._create_weighted_choices([(currency, pct) for currency, pct in currency_data.items()])
        ...
        for batch_start in range(0, count, batch_size):
            ...
            try:
                with transaction.atomic():
                    for i in range(batch_start, batch_end):
                        try:
                            ...
                            errors += 1
                            self.stdout.write(self.style.ERROR(f"Error generating sale {i+1}: {str(e)}"))
                ...
                self.stdout.write(self.style.ERROR(f"Error creating batch {batch_start}-{batch_end}: {str(e)}"))
                errors += len(batch_sales)

        # Final report
        self.stdout.write(self.style.SUCCESS(f"Successfully created {sales_created} marketplace sales"))
    ...
    def _generate_sale_data(
        self, users, products, anonymous_ratio, date_range_days, status_choices, payment_status_choices, currency_choices
    ):
    ...
    def _show_creation_stats(self):
        ...
        total_revenue = MarketplaceSale.objects.aggregate(total=models.Sum("total_amount"))["total"] or 0

        status_stats = MarketplaceSale.objects.values("status").annotate(count=models.Count("id"))

        payment_stats = MarketplaceSale.objects.values("payment_status").annotate(count=models.Count("id"))
