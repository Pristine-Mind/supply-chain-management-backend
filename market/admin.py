...
class MarketplaceOrderAdmin(RoleBasedModelAdminMixin, admin.ModelAdmin):
    ...
    def mark_as_confirmed(self, request, queryset):
        ...
        self.message_user(
            request, 
            f"{count} order(s) marked as confirmed."
        )
    ...
    def mark_as_delivered(self, request, queryset):
        ...
        self.message_user(
            request, 
            f"{count} order(s) marked as delivered."
        )
...
class OrderTrackingEventAdmin(RoleBasedModelAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Order Reference"), {
            "fields": ("marketplace_sale", "marketplace_order")
        }),
        (_("Event Details"), {
            "fields": ("status", "message", "location")
        }),
        (_("Location Data"), {
            "fields": ("latitude", "longitude"),
            "classes": ("collapse",)
