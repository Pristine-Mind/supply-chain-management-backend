...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                report, created = DailySalesReport.objects.get_or_create(
                    report_date=report_date, defaults={"total_items": 0, "total_revenue": Decimal("0.00")}
                )
                if not created and not options["force"]:
                    self.stdout.write(
                        self.style.WARNING(f"Report for {report_date} already exists. Use --force to override.")
                    )
                    return
                ...
                qs = (
                    CartItem.objects.select_related("product__product__user")
                ...
                if not items:
                    self.stdout.write(self.style.WARNING(f"No sales for {report_date}; created empty report."))
                else:
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Report {report_date} generated: {total_items} items, revenue {total_revenue}")
        )
