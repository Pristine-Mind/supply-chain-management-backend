...
def fetch_data_in_chunks(endpoint_url: str, headers: dict = None, chunk_size: int = 50, sleep_time: float = 1.0, is_mock: bool = True):
    all_results = []
    offset = 0
        ...
        if is_mock:
            ...
            for i in range(offset, min(offset + chunk_size, offset + 5)): 
                if "categories" in endpoint_url:
                    results.append({"id": i, "code": f"C{i}", "name": f"Mock Category {i}", "description": "", "is_active": True, "created_at": "2025-11-06T00:00:00Z", "updated_at": "2025-11-06T00:00:00Z", "subcategories_count": 3})
                    ...
                    results.append({"id": i, "name": f"Mock Brand {i}", "description": "", "country_of_origin": "", "is_active": True, "is_verified": False, "products_count": 10})
                elif "products" in endpoint_url:
                    results.append({"id": i, "name": f"Mock Product {i}", "category_details": "Home", "description": "<p>Test product</p>", "price": 100.0, "stock": 20, "producer": 1, "brand_name": "Mock Brand"})
                elif "direct-sales" in endpoint_url:
                    results.append({"id": i, "product": random.randint(1, 10), "quantity": random.randint(1, 5), "unit_price": 150.0, "sale_date": "2026-08-03T10:00:00Z"})
                elif "erp-orders" in endpoint_url:
                    results.append({"id": i, "quantity": random.randint(10, 50), "total_price": 5000.0, "payment_status": random.choice(["cash", "online"]), "delivery_date": "2026-08-05T10:00:00Z"})
                elif "ecommerce-sales" in endpoint_url:
                    results.append({"id": i, "buyer": 100 + random.randint(1, 5), "seller": 100 + random.randint(6, 10), "product": random.randint(1, 10), "quantity": random.randint(1, 3), "payment_status": "online", "shipping_cost": 50.0})
                elif "b2b-interactions" in endpoint_url:
                    int_type = random.choice(list(interaction_types.keys()))
                    results.append({"id": i, "initiator": 100 + random.randint(1, 5), "target_business": 100 + random.randint(6, 10), "interaction_type": int_type, "weight": interaction_types[int_type]})
...
if __name__ == "__main__":
    ...
    direct_sales = get_cached_data("https://appmulyabazzar.com/api/v1/direct-sales/", "direct_sales_cache.json", is_mock=True)
    erp_orders = get_cached_data("https://appmulyabazzar.com/api/v1/erp-orders/", "erp_orders_cache.json", is_mock=True)
    eco_sales = get_cached_data("https://appmulyabazzar.com/api/v1/ecommerce-sales/", "ecommerce_sales_cache.json", is_mock=True)
