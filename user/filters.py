...
class BusinessFilter(django_filters.FilterSet):
    ...
    business_type = django_filters.ChoiceFilter(
        ...
        help_text="Filter by business type (distributor/retailer)"
    )
    ...
    b2b_verified = django_filters.BooleanFilter(
        help_text="Filter by B2B verification status"
    )
    
    has_marketplace_access = django_filters.BooleanFilter(
        field_name='has_access_to_marketplace',
        help_text="Filter by marketplace access"
    )
    ...
    min_credit_limit = django_filters.NumberFilter(
        ...
        lookup_expr='gte',
        help_text="Minimum credit limit"
    )
    ...
    registered_after = django_filters.DateTimeFilter(
        ...
        lookup_expr='gte',
        help_text="Show businesses registered after this date"
    )
    
    registered_before = django_filters.DateTimeFilter(
        field_name='user__date_joined',
        lookup_expr='lte',
        help_text="Show businesses registered before this date"
    )
    
    # Search filters
    search = django_filters.CharFilter(
        method='filter_search',
        help_text="Search in business name, user name, or phone number"
    )
    
    # Geographic distance filtering
    latitude = django_filters.NumberFilter(
        method='filter_by_distance',
        help_text="Latitude for distance-based filtering (use with longitude and radius)"
    )
    
    longitude = django_filters.NumberFilter(
        method='filter_by_distance', 
        help_text="Longitude for distance-based filtering (use with latitude and radius)"
    )
    
    radius_km = django_filters.NumberFilter(
        method='filter_by_distance',
        help_text="Radius in kilometers for distance-based filtering"
    )

    class Meta:
        model = UserProfile
        fields = [
            'business_type', 'city', 'city_name', 'b2b_verified', 
            'has_marketplace_access', 'is_active', 'min_credit_limit', 
            'max_credit_limit', 'registered_after', 'registered_before',
