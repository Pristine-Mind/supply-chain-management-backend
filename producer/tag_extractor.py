...
class SemanticTagExtractor:
    ...
    def extract_semantic_tags(
        ...
        if not candidate_tags or not product_text.strip():
            return []

        model = cls.get_model()
        product_embedding = model.encode(
            product_text, convert_to_tensor=True, normalize_embeddings=True
        )
        tag_embeddings = model.encode(
            candidate_tags, convert_to_tensor=True, normalize_embeddings=True
        )
        similarities = util.cos_sim(product_embedding, tag_embeddings)[0]

        matched_tags = []
        for idx, score in enumerate(similarities):
            if score.item() >= threshold:
                matched_tags.append((candidate_tags[idx], score.item()))
        matched_tags.sort(key=lambda x: x[1], reverse=True)
        return [tag for tag, _ in matched_tags[:top_k]]


class TagExtractor:
    """Comprehensive tag extraction for marketplace products"""

    MASTER_CANDIDATE_TAGS = [
        "refrigerator", "fridge", "freezer", "cooling appliance", "home appliances",
        "kitchen appliances", "single door", "double door", "inverter", "energy efficient",
        "smart tv", "television", "electronics", "washing machine", "air conditioner",
        "mobile phone", "laptop", "home living", "kitchenware", "furniture", "apparel"
    ]
    ...
    CATEGORY_TAGS = {
        "EG": { 
            "products": {
                "television_tv": [
                    "tv", "television", "led tv", "lcd tv", "oled tv", "qled tv",
                    "4k tv", "8k tv", "smart tv", "android tv", "google tv", "roku tv",
                    "curved tv", "flat tv", "hd ready", "full hd", "ultra hd", "hdr tv",
                    "dolby vision",
                ],
                "refrigerator_fridge": [
                    "refrigerator", "refrigerato", "fridge", "freezer", "mini fridge", 
                    "double door", "single door", "side by side", "french door", 
                    "bottom freezer", "top freezer", "frost free", "direct cool", "inverter fridge",
                ],
                "washing_machine": [
                    "washing machine", "washer", "dryer", "washer dryer", "front load",
                    "top load", "semi automatic", "fully automatic", "portable washer",
                ],
                "air_conditioner": [
                    "ac", "air conditioner", "split ac", "window ac", "portable ac",
                    "inverter ac", "smart ac", "1 ton", "1.5 ton", "2 ton",
                ],
            ...
            "attributes": {
                "display": [
                    "oled", "amoled", "lcd", "ips", "retina", "hdr", "4k", "8k",
                    "1080p", "720p", "120hz", "144hz", "240hz", "touch screen", "foldable",
                ],
        ...
        "HL": {  
            "products": {
                ...
                "kitchen_appliances": [
                    "microwave", "oven", "stove", "cooktop", "chimney", "water heater",
                    "geyser", "mixer", "grinder", "juicer", "blender", "air fryer",
                    "rice cooker", "toaster", "kettle", "coffee maker",
                ],
                "furniture": [
                    "sofa", "chair", "table", "bed", "mattress", "pillow", "wardrobe",
                    "cabinet", "bookshelf", "dining table", "office chair", "study table",
                ],
        ...
        "FA": {  
            "products": {
                "clothing": [
                    "shirt", "t shirt", "pants", "jeans", "jacket", "dress", "skirt",
                    "sweater", "hoodie", "coat", "blouse", "leggings", "shorts", "saree",
                    "kurti", "sherwani",
                ],
                "footwear": ["shoes", "sneakers", "sandals", "slippers", "boots", "heels", "flats", "loafers"],
                "accessories": [
                    "watch", "bag", "belt", "hat", "scarf", "sunglasses", "jewelry",
                    "necklace", "earrings", "ring", "bracelet", "wallet",
                ],
            ...
            "attributes": {
                "material": [
                    ...
                    "nylon", "cashmere", "velvet", "satin", "lace",
                ],
        ...
        "SP": {  
            ...
            "attributes": {
                "sport_type": [
                    ...
                    "running", "yoga", "gym", "workout", "cardio", "strength",
                ],
    ...
    BRANDS = {
        "electronics": [
            ...
            "xiaomi", "oneplus", "realme", "apple", "google", "microsoft", "dell",
            "hp", "lenovo", "asus", "acer", "msi", "razer", "logitech", "bose",
            "jbl", "boat", "mi", "redmi", "oppo", "vivo",
        ],
        "appliances": [
            "hismstar", "whirlpool", "godrej", "haier", "voltas", "daikin", "blue star", 
            "carrier", "hitachi", "mitsubishi", "bosch", "siemens", "electrolux", 
            "ifb", "videocon", "crompton", "havells",
        ],
        "fashion": [
            "nike", "adidas", "puma", "reebok", "zara", "hm", "levi", "arrow",
            "allen solly", "us polo", "tommy", "calvin klein", "gucci", "prada",
            "louis vuitton", "chanel",
        ],
        "grocery": [
            "nestle", "unilever", "procter & gamble", "pepsico", "coca cola",
            "britannia", "amul", "dabur", "patanjali", "mother dairy",
        ],
    }
    ...
    def extract_tags(cls, marketplace_product):
        ...
        ai_tags = SemanticTagExtractor.extract_semantic_tags(
            product_text=combined_text,
            candidate_tags=cls.MASTER_CANDIDATE_TAGS,
            top_k=8,
            threshold=0.38,
        )
        tags.update(ai_tags)
    ...
    def _extract_specifications(cls, text, category_code):
        ...
        capacity_matches = re.findall(r"(\d+(?:\.\d+)?)\s*(liter|liters|ltr|ltrs|litre|litres|kg|kgs|ton)", text, re.IGNORECASE)
        for capacity, unit in capacity_matches:
            tags.add(f"{capacity}{unit}")
            capacity_float = float(capacity)
            unit_lower = unit.lower()

            if unit_lower in ["liter", "liters", "ltr", "ltrs", "litre", "litres"]:
    ...
    def _extract_colors(cls, text, marketplace_product):
        ...
        if marketplace_product.color:
            model_cls = type(marketplace_product)
            
            if hasattr(model_cls, 'ColorChoices'):
                color_display = dict(model_cls.ColorChoices.choices).get(marketplace_product.color, "")
                if color_display:
        ...
        common_colors = [
            "red", "blue", "green", "black", "white", "gray", "grey", "brown",
            "orange", "purple", "pink", "navy", "beige", "gold", "silver",
            "yellow", "maroon", "cyan", "magenta", "violet", "indigo",
        ]
