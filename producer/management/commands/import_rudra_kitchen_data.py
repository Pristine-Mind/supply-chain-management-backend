...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(excel_file_path):
            self.stdout.write(
                self.style.ERROR(f'Excel file not found: {excel_file_path}')
            )
            return
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if profile_created:
                    ...
                    user_profile.save()
                    self.stdout.write(
                        self.style.WARNING(f'Updated user profile for: {username}')
                    )
                ...
                if producer_created:
                    self.stdout.write(
                        self.style.SUCCESS(f'Created producer: {producer.name}')
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(f'Producer already exists: {producer.name}')
                    )
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f'Created category: {home_living_category.name}')
                    )
                ...
                for index, row in df.iterrows():
                    try:
                        ...
                        if not product_name or product_name.lower() == 'nan' or len(product_name.strip()) == 0:
                            self.stdout.write(
                                self.style.WARNING(f'Row {row_num}: Skipping empty product name')
                            )
                            continue
