...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not producers:
            self.stdout.write(
                self.style.WARNING("No producers found with the specified IDs")
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Successfully exported data for {len(producers)} producers")
        )
    ...
    def _export_to_excel(self, producers, discount_percentage, output_file=None):
        ...
        summary_headers = [
            "Producer ID", "Producer Name", "Email", "Contact", 
            "Registration Number", "Total Products", "Total Stock Value"
        ]
        ...
        for producer in producers:
            ...
            total_stock_value = sum(
                (product.price * product.stock) for product in producer.products
                if product.price and product.stock
            ) if producer.products else 0
            
            # Add to summary
            summary_data = [
                producer.id, producer.name, producer.email, producer.contact,
                producer.registration_number, total_products, f"${total_stock_value:.2f}"
            ]
            ...
            # Product headers
            product_headers = [
                "Product ID", "Product Name", "Brand", "SKU", "Category",
                "Cost Price", "Selling Price", f"Discounted Price ({discount_percentage}%)", 
                "Stock", "Reorder Level", "Size", "Color", "Status", "Created At", "Image Count"
            ]
